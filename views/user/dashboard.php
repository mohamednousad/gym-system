...
if (is_post()) {
    ...
    $action = post('action');
    if ($action === 'checkin') {
        $exists = $pdo->prepare('SELECT id FROM attendance WHERE user_id=:uid AND attendance_date=CURDATE()'); $exists->execute(['uid'=>$user['id']]); 
        if ($exists->fetch()) { flash('Already checked in today.','error'); }
        else {
...
$today = $pdo->prepare('SELECT * FROM attendance WHERE user_id=:uid AND attendance_date=CURDATE()'); $today->execute(['uid'=>$user['id']]); $today=$today->fetch();
$active_wp = $pdo->prepare('SELECT uwp.*,w.plan_name,w.goal,w.difficulty,t.name trainer FROM user_workout_plans uwp JOIN workout_plans w ON w.id=uwp.workout_plan_id LEFT JOIN trainers t ON t.id=w.trainer_id WHERE uwp.user_id=:uid AND uwp.status="active" ORDER BY uwp.id DESC LIMIT 1'); $active_wp->execute(['uid'=>$user['id']]); $active_wp=$active_wp->fetch();
$recent_att = $pdo->prepare('SELECT * FROM attendance WHERE user_id=:uid ORDER BY attendance_date DESC LIMIT 6'); $recent_att->execute(['uid'=>$user['id']]); $recent_att=$recent_att->fetchAll();
$notes = $pdo->prepare('SELECT * FROM notifications WHERE user_id=:uid AND is_read=0 ORDER BY created_at DESC LIMIT 5'); $notes->execute(['uid'=>$user['id']]); $notes=$notes->fetchAll();
...
<div class="quick-bar">
  ...
  <form method="POST">
    ...
    <?php else: ?>
      <span class="badge badge-success" style="font-size:13px;padding:8px 14px;">&#10003; Completed Today</span>
    <?php endif; ?>
  </form>
</div>
...
<div class="stat-cards">
  <div class="stat-card">
    ...
    <div class="stat-val" style="font-size:20px;"><?= e($user['membership_plan'] ?: 'None') ?></div>
  ...
  <div class="stat-card">
    ...
    <div class="stat-val" style="font-size:18px;"><?= e($user['renewal_date'] ?: '—') ?></div>
    <div class="stat-label">Renewal Date</div>
  </div>
  <div class="stat-card">
    <div class="stat-icon" style="background:rgba(34,197,94,0.12);color:var(--success);">&#127947;</div>
    <div class="stat-val" style="font-size:18px;"><?= e($active_wp['plan_name'] ?? 'None') ?></div>
...
<div class="grid-2">
  ...
  <div class="card">
    ...
    <?php endforeach; ?>
    <?php if(empty($notes)): ?><div class="empty-state"><p>All caught up &#127881;</p></div><?php endif; ?>
  </div>
</div>
